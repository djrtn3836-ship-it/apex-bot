"""APEX BOT Backtester - Upbit   
Upbit REST API  OHLCV   ."""
import time
import asyncio
import aiohttp
import pandas as pd
...
async def fetch_ohlcv(
    ...
    if interval not in INTERVAL_MAP:
        raise ValueError(f"  interval: {interval}. : {list(INTERVAL_MAP)}")

    _, minutes_per_bar = INTERVAL_MAP[interval]
    total_bars = max(1, (days * 1440) // minutes_per_bar)
    logger.info(f"[DataLoader] {market} {interval} {total_bars}  ...")
    ...
    if not all_rows:
        logger.warning(f"[DataLoader] {market} {interval}  ")
        return pd.DataFrame()
