...
class RiskManager:
    ...
    def get_kelly_params(self) -> Dict:
        ...
        # [BUG-3 FIX] DB 실거래 데이터 기반 avg_win/avg_loss 동적 계산
        avg_win  = 0.03   # 기본값
        avg_loss = 0.02   # 기본값
        try:
            import sqlite3
            from pathlib import Path as _P
            _db = _P("database/apex_bot.db")
            if _db.exists():
                con = sqlite3.connect(str(_db))
                cur = con.cursor()
                cur.execute(
                    "SELECT profit_rate FROM trade_history WHERE side='SELL' ORDER BY id DESC LIMIT 30"
                )
                rows = [r[0] for r in cur.fetchall() if r[0] is not None]
                con.close()
                if len(rows) >= 5:
                    wins_r   = [r      for r in rows if r > 0]
                    losses_r = [abs(r) for r in rows if r < 0]
                    if wins_r:
                        avg_win  = sum(wins_r)   / len(wins_r)
                    if losses_r:
                        avg_loss = sum(losses_r) / len(losses_r)
                    logger.info(
                        f"[Kelly-dynamic] avg_win={avg_win:.4f} "
                        f"avg_loss={avg_loss:.4f} (n={len(rows)}건)"
                    )
        except Exception as _ke:
            logger.debug(f"[Kelly-dynamic] DB 조회 실패: {_ke}")
        kelly = (wr * avg_win - (1 - wr) * avg_loss) / avg_win if avg_win > 0 else 0.1
        kelly = max(0.05, min(kelly, 0.20))
        logger.debug(
            f"[Kelly] wr={wr:.3f} avg_win={avg_win:.4f} "
            f"avg_loss={avg_loss:.4f} kelly={kelly:.4f}"
        )
