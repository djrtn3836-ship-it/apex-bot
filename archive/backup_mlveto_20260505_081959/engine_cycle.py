    ...
    async def _analyze_existing_position(self, market: str) -> None:
        try:
            ...
            if _held_min < 10 and pnl_pct > -2.0:  # 30→10분으로 완화
                ...
                (signal == "SELL" and confidence >= 0.65 and pnl_pct >= 0.5) or   # ML 익절 최소 +0.5%
                (confidence >= 0.65 and pnl_pct >= 1.5) or                        # 강한 수익 익절
                (confidence >= 0.65 and pnl_pct <= -1.5) or                       # ML 신뢰도 기반 손절
                (pnl_pct >= 3.0) or                                                # 3% 무조건 익절
                (pnl_pct <= -2.0 and (confidence >= 0.50 or _held_min >= 720)) or # 12h+ 보유 시 confidence 완화
