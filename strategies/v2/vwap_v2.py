...
class VWAPReversionStrategy2(BaseStrategy):
    ...
    def generate_signal(self, df: pd.DataFrame, market: str = "") -> Optional[Signal]:
        ...
        try:
            ...
            ).get('strategies', {}).get('VWAP_Reversion', {})
            ...
            if _fgi < _fg_max:
                logger.debug(f'[FG-BLOCK] VWAP_Reversion FG={_fgi}<{_fg_max} → 신호 무시')
                return None
        except Exception:
    ...
    def _check_entry(
        ...
        for avwap in avwaps:
            ...
            stop_loss      = safe_last(df["close"]) * (1 - _SETTINGS.STRATEGY_SL_RATIO.get("VWAP_Reversion", 0.017)),
