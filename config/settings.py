    ...
    def validate(self):
        ...
        if self.mode == "live":
            ...
            if confirm != "yes":
                raise RuntimeError(
                    "\n" + "=" * 55 + "\n"
                    "  Í≤ΩÍ≥†: ?§Í±∞??Î™®Îìú???òÍ≤Ω Î≥Ä???ïÏù∏ ?ÑÏöî\n"
                    "  ?òÍ≤ΩÎ≥Ä?òÏóê ?§Ï†ï?òÏÑ∏??\n"
                    "  APEX_LIVE_CONFIRM=yes\n"
                    "  (?§Í±∞?òÏùò ?ÑÌóò?±ÏùÑ .env??Î™ÖÏãú)\n"
                    + "=" * 55
                )
            assert self.api.access_key, "UPBIT_ACCESS_KEY ?ÑÏàò"
            assert self.api.secret_key, "UPBIT_SECRET_KEY ?ÑÏàò"

        assert 0 < self.risk.max_risk_per_trade <= 0.05, (
            "max_risk_per_trade??0~5% ?¨Ïù¥"
        )
